...
function init()
    ctx = ConstantContext(0, Array{Float64,2}(undef, 0, 0), Array{Float64,1}(undef, 0), Array{Int64,1}(undef, 0), Array{Float64,4}(undef, 0, 0, 0, 0))
    return ctx
end
...
function init!(ctx::ConstantContext, notd)
    ...
    for n in 0:(nbc - 1)
        ...
        bcof[n + 1, n + 1] =
            fnr[2n + 2] * fnr[2n + 1] * bcof[n, n] / fnr[n + 1] / fnr[n + 1]
    end
